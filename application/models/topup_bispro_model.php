...
class Topup_bispro_model extends Base_Model {
    ...
    function topup($data){
        ...
        $norek_bispro = $this->rekening_model->get_norek_member($data['id_member'], 'BISPRO');
        ...
        if($data['id_bank']=='WALLET'){
            ...
            $this->db->insert('t_rek_wallet', array(
                ...
                'norek' => $norek_wallet,
                'jenis_trx' => 'TRF',
                'ket' => 'TOPUP BISPRO TO '.$norek_bispro.': '.$member->id.' '.$member->nama,
                'dk' => 'D',
                'nominal' => $nominal,
        ...
        //2. update saldo
        $this->db->where('norek', $norek_bispro);
        $this->db->set('saldo', 'saldo+'.$nominal, false);
        $this->db->update('m_rekening');
    ...
    function validasi($data){
        ...
        $this->load->model('rekening_model');
        $norek = $this->rekening_model->get_norek_member($data['id_member'], 'BISPRO');
        $this->load->model('member_model');
        $member = $this->member_model->get_data($data['id_member']);

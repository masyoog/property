...
class Setoran_wallet extends Site_Controller {
    ...
    function index(){
        $data = array(
            "title" => "Setoran Rekening E-Wallet",
    ...
    function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $create = $this->my_model->setoran($data);
            if ($create) {
                ...
                $this->email->subject('[PRO2M] SETORAN DANA E-WALLET');
                $msg = '
                    Dear '.$member->nama.' ('.$member->id.'),
                        
                    Selamat saldo rekening E-WALLET Anda telah berhasil ditambahkan sebesar Rp '._format_angka($data['nominal']).'.
                ...
                $this->outbox_model->send_sms(array(
                    ...
                    'pesan' => 'Yth.'.$member->nama.' ('.$member->id.'), selamat saldo rekening E-WALLET Anda telah berhasil ditambahkan sebesar Rp '._format_angka($data['nominal']),
                    'type' => '1',
                    'port' => '0',
                    'status' => '0'
                ));

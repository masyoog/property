...
class Validasi_topup_bispro extends Site_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model('topup_bispro_model', 'my_model');
    ...
    function index() {
        $this->init(false);
        $data = array(
            "title" => "Validasi Konfirmasi Topup Dana BISPRO",
            'subtitle' => 'List Data',
            'css' => array(
    ...
    function validasi(){
        $this->init();
        $data = array(
            "title" => "Validasi Topup Dana BISPRO",
    ...
    function validate() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($create) {
                ...
                $this->email->subject('[PRO2M] KONFIRMASI TOPUP DANA BISPRO');
                $msg = '
                    Dear '.$member->nama.' ('.$member->id.'),
                        
                    Selamat saldo rekening BISPRO Anda telah berhasil ditambahkan sebesar Rp '._format_angka($data['nominal']).'.
                ...
                $this->outbox_model->send_sms(array(
                    ...
                    'pesan' => 'Yth.'.$member->nama.' ('.$member->id.'), selamat saldo rekening BISPRO Anda telah berhasil ditambahkan sebesar Rp '._format_angka($data['nominal']),
                    'type' => '1',
                    'port' => '0',
                    'status' => '0'
                ));

...
class Saham_ref_bispro extends Site_Controller {
    ...
    function get_data(){
        $this->datatables->select('m.id as id_member, m.nama as nama, sum(r2.nominal) as saham_sudah_dibagi, '
                . 'sum(r.nominal) as saham_pending, m.id as action')
                ->add_column('action', $this->get_action_button('$1'), 'id_member')
                ->edit_column('saham_sudah_dibagi', '$1', '_format_angka(saham_sudah_dibagi)')
                ->edit_column('saham_pending', '$1', '_format_angka(saham_pending)')
                ->from("m_member m")
//                ->where('m.status ', '1')
                ->join('v_komisi_bispro_0 r', 'm.id=r.id_member', 'left')
                ->join('v_komisi_bispro_1 r2', 'm.id=r2.id_member', 'left')
                ->group_by('m.id')
                ;
    ...
    function get_action_button($param){
        ...
        $ret='<a title="Lihat Mutasi" class="btn btn-primary btn-xs btn-floating-action" href="'.base_url().$this->class_name.'/mutasi/'.$param.'"><i class="glyphicon glyphicon-list"></i></a> &nbsp;
            <a title="Print Sertifikat" class="btn btn-warning btn-xs btn-floating-action" href="'.base_url().$this->class_name.'/print/'.$param.'"><i class="glyphicon glyphicon-print"></i></a>
        ';
        return $ret;
    }
    function index() {
        ...
        foreach($this->field as $key => $val){
            ...
            if($key=='saham_sudah_dibagi' || $key=='saham_pending'){
    ...
    private function init(){
        $this->field = array(
            ...
            'saham_sudah_dibagi' => array(
                'label'=>'saham sudah dibagi',
            ),
            'saham_pending' => array(
                'label'=>'saham aktif',
            ),

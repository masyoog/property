...
class Jenis_produk extends Site_Controller {
    ...
    function get_data(){
        $this->datatables->select('id as kode, nama, id AS action')
    ...
    function detail(){
        ...
        $data['datalist'] = $this->jenis_produk_model->get_data($id, $this->key_id);
    ...
    function edit() {
        ...
        $res = $this->jenis_produk_model->get_data($id, $this->key_id);
        if ($res == null) {
            $this->session->set_flashdata('msg_err', 'Data not found');
            redirect('user');
        } else {
    ...
    function delete() {
        $id = $this->security->xss_clean($this->uri->segment(3));
        $result = $this->jenis_produk_model->get_data($id);
        if ($result == null) {
            $this->session->set_flashdata('msg_err', 'Data not found');
            redirect($this->class);
        } else {
            $delete = $this->jenis_produk_model->delete_data($id, $this->key_id);
            if ($delete) {
    ...
    function insert() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $create = $this->jenis_produk_model->insert_data($data);
            if ($create) {
                $this->session->set_flashdata('msg', 'Data saved successfully');
            } else {
    ...
    function update() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $update = $this->jenis_produk_model->update_data($id, $data, $this->key_id);
            if ($update) {
                $this->session->set_flashdata('msg', 'Data saved successfully');
            } else {
                $this->session->set_flashdata('msg_err', 'Failed to save data');
    ...
    function cek_double_id(){
        ...
        $res = $this->jenis_produk_model->get_total_data("(id='$kode')", $this->key_id);
        if($res>0){

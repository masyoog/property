...
class Saham_bispro extends Site_Controller {
    ...
    function get_data(){
        $this->datatables->select('m.id as id_member, m.nama as nama, r.saldo as saldo_bispro, m.id as action')
                ->add_column('action', $this->get_action_button('$1'), 'id_member')
                ->edit_column('saldo_bispro', '$1', '_format_angka(saldo_bispro)')
                ->from("m_member m")
//                ->where('m.status ', '1')
                ->join('v_rekening_bispro r', 'm.id=r.id_member', 'left')
                ;
